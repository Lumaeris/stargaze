...
build $image_name=image_name $image_tag=image_tag $ci=ci:
    #!/usr/bin/env bash
    LABELS=()
    if [[ {{ci}} == "true" ]]; then
        # Labels
    ...
    {{container_runtime}} build "${LABELS[@]}" -t "${image_name}:${image_tag}" .
...
rechunk $image_name=image_name $image_tag=image_tag:
    #!/usr/bin/env bash
